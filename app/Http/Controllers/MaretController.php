...
use App\Models\Maret;
use App\Models\Santri;



class MaretController extends Controller
{
    function index(){
        ...
        $data['sudah'] =  DB::table('maret')
        	->count();

        $data['belum'] =  DB::table('maret')
         	->select (DB::raw('COUNT(santri.id_santri) as belum_lunas'))
         	->rightjoin('santri', 'maret.id_santri', '=', 'santri.id')
         	->where('sts_santri','=','aktif')
         	->where('jk','=','pria')
         	->where('maret.nominal')
        	->get();
        $data['belumSpp50'] =  DB::table('maret')
         	...
         	->where('maret.nominal')
        	->get();
        $data['belumSpp300'] =  DB::table('maret')
		...
		$data['index'] = DB::table('maret')
            ->select('maret.*', 'santri.id_santri', 'santri.nama_santri','santri.sts_ads_spp',
            	'administrasi.nama_administrasi', 'maret.nominal', 'user.username')
            ->join('santri', 'maret.id_santri', '=', 'santri.id')
            ->join('user', 'maret.id_user', '=', 'user.id')
			->join('administrasi', 'maret.id_administrasi', '=', 'administrasi.id')
			->latest()
            ->get();

       	$data['tunggakan'] = DB::table('maret')
            ->select('santri.id_santri', 'santri.nama_santri', 'santri.sts_ads_spp', 'maret.nominal', 'maret.status')
            ->rightjoin('santri', 'maret.id_santri', '=', 'santri.id')
        ...
        $data['putra_spp300'] = DB::table('santri')
            ...
            ->get();
		return view('admin.pembayaran.spp.maret.index', $data);
	}
	...
	function create(){
		$data['list_santri'] = DB::table('santri')
        	...
        	->where('jk','=','pria')
        	->get();
		return view('admin.pembayaran.spp.maret.create',$data);
	}

	function store(){
		$maret = new maret;
		$maret->id_santri = request('id_santri');
		$maret->id_administrasi = request('id_administrasi');
		$maret->id_user = request('id_user');
		$maret->nominal = request('nominal');
		$maret->waktu = request('waktu');
		$maret->status = request('status');
		$maret->keterangan = request('keterangan');

		$maret->save();

		return redirect('admin/maret')->with('success','Data Berhasil Ditambahkan');
		dd(request()->all());
	}

	function show(maret $maret){
		$data['maret'] = $maret;
		return view('admin.pembayaran.spp.maret.show', $data);
	}

	function edit(maret $maret){
		$data['maret'] = $maret;
		return view('admin.pembayaran.spp.maret.edit', $data);
	}

	function update(maret $maret){
		$maret->id_santri = request('id_santri');
		$maret->id_administrasi = request('id_administrasi');
		$maret->id_user = request('id_user');
		$maret->nominal = request('nominal');
		...
		return redirect('admin/maret')->with('success','Data Berhasil Diubah');
	}

	function destroy(maret $maret){
		$maret->delete();
		return redirect('admin/maret')->with('danger','Data Berhasil Dihapus');
	}


	function kwitansiThermo(maret $maret){
		$data['maret'] = $maret;
		return view('admin.pembayaran.spp.maret.kwitansiThermo', $data);
	}

	function kwitansiTinta(maret $maret){
		$data['maret'] = $maret;
		return view('admin.pembayaran.spp.maret.kwitansiTinta', $data);
	}
