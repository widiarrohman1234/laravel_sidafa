...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-agustus</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP agustus Index</button>
	<a href="{{url('admin/agustus/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/agustus/kwitansiThermo', $agustus->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/agustus/kwitansiTinta', $agustus->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/agustus', $agustus->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/agustus', $agustus->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/agustus', $agustus->id)])
			</div>
			</td>
			<td>{{$agustus->id_santri}}</td>
			<td>{{$agustus->nama_santri}}</td>
			<td>{{$agustus->sts_ads_spp}}</td>
			<td>Rp.{{number_format($agustus->nominal)}}</td>
			<td>{{$agustus->waktu}}</td>
			<td>@if($agustus->status == 'lunas')
			<button type="button" class="btn btn-success">{{$agustus->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$agustus->username}}</td>
			<td>{{$agustus->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP agustus Tunggakan</button>
	<a href="{{url('admin/agustus/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
