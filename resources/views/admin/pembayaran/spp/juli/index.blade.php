...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-juli</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP juli Index</button>
	<a href="{{url('admin/juli/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/juli/kwitansiThermo', $juli->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/juli/kwitansiTinta', $juli->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/juli', $juli->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/juli', $juli->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/juli', $juli->id)])
			</div>
			</td>
			<td>{{$juli->id_santri}}</td>
			<td>{{$juli->nama_santri}}</td>
			<td>{{$juli->sts_ads_spp}}</td>
			<td>Rp.{{number_format($juli->nominal)}}</td>
			<td>{{$juli->waktu}}</td>
			<td>@if($juli->status == 'lunas')
			<button type="button" class="btn btn-success">{{$juli->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$juli->username}}</td>
			<td>{{$juli->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP juli Tunggakan</button>
	<a href="{{url('admin/juli/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
