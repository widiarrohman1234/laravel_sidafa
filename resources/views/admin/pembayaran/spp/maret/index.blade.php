...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-maret</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP maret Index</button>
	<a href="{{url('admin/maret/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/maret/kwitansiThermo', $maret->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/maret/kwitansiTinta', $maret->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/maret', $maret->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/maret', $maret->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/maret', $maret->id)])
			</div>
			</td>
			<td>{{$maret->id_santri}}</td>
			<td>{{$maret->nama_santri}}</td>
			<td>{{$maret->sts_ads_spp}}</td>
			<td>Rp.{{number_format($maret->nominal)}}</td>
			<td>{{$maret->waktu}}</td>
			<td>@if($maret->status == 'lunas')
			<button type="button" class="btn btn-success">{{$maret->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$maret->username}}</td>
			<td>{{$maret->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP maret Tunggakan</button>
	<a href="{{url('admin/maret/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
