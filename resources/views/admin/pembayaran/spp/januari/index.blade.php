...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-JANUARI</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP Januari Index</button>
	<a href="{{url('admin/januari/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/januari/kwitansiThermo', $januari->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/januari/kwitansiTinta', $januari->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/januari', $januari->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/januari', $januari->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/januari', $januari->id)])
			</div>
			</td>
			<td>{{$januari->id_santri}}</td>
			<td>{{$januari->nama_santri}}</td>
			<td>{{$januari->sts_ads_spp}}</td>
			<td>Rp.{{number_format($januari->nominal)}}</td>
			<td>{{$januari->waktu}}</td>
			<td>@if($januari->status == 'lunas')
			<button type="button" class="btn btn-success">{{$januari->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$januari->username}}</td>
			<td>{{$januari->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP Januari Tunggakan</button>
	<a href="{{url('admin/januari/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
