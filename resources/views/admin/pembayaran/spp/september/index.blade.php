...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-september</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP september Index</button>
	<a href="{{url('admin/september/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/september/kwitansiThermo', $september->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/september/kwitansiTinta', $september->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/september', $september->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/september', $september->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/september', $september->id)])
			</div>
			</td>
			<td>{{$september->id_santri}}</td>
			<td>{{$september->nama_santri}}</td>
			<td>{{$september->sts_ads_spp}}</td>
			<td>Rp.{{number_format($september->nominal)}}</td>
			<td>{{$september->waktu}}</td>
			<td>@if($september->status == 'lunas')
			<button type="button" class="btn btn-success">{{$september->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$september->username}}</td>
			<td>{{$september->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP september Tunggakan</button>
	<a href="{{url('admin/september/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
			...
			<tbody>
				@foreach($tunggakan as $september)
				 <tr>
					<td>{{$loop->iteration}}</td>
					<td>{{$september->id_santri}}</td>
					<td>{{$september->nama_santri}}</td>
