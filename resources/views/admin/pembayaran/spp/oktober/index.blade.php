...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-oktober</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP oktober Index</button>
	<a href="{{url('admin/oktober/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/oktober/kwitansiThermo', $oktober->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/oktober/kwitansiTinta', $oktober->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/oktober', $oktober->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/oktober', $oktober->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/oktober', $oktober->id)])
			</div>
			</td>
			<td>{{$oktober->id_santri}}</td>
			<td>{{$oktober->nama_santri}}</td>
			<td>{{$oktober->sts_ads_spp}}</td>
			<td>Rp.{{number_format($oktober->nominal)}}</td>
			<td>{{$oktober->waktu}}</td>
			<td>@if($oktober->status == 'lunas')
			<button type="button" class="btn btn-success">{{$oktober->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$oktober->username}}</td>
			<td>{{$oktober->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP oktober Tunggakan</button>
	<a href="{{url('admin/oktober/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
