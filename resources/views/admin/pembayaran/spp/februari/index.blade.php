...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-februari</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<p>Sudah = <button type="button" class="btn btn-success">{{$sudah[0]->lunas+1}} orang</button></p>
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP februari Index</button>
	<a href="{{url('admin/februari/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/februari/kwitansiThermo', $februari->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/februari/kwitansiTinta', $februari->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/februari', $februari->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/februari', $februari->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/februari', $februari->id)])
			</div>
			</td>
			<td>{{$februari->id_santri}}</td>
			<td>{{$februari->nama_santri}}</td>
			<td>{{$februari->sts_ads_spp}}</td>
			<td>Rp.{{number_format($februari->nominal)}}</td>
			<td>{{$februari->waktu}}</td>
			<td>@if($februari->status == 'lunas')
			<button type="button" class="btn btn-success">{{$februari->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$februari->username}}</td>
			<td>{{$februari->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP februari Tunggakan</button>
	<a href="{{url('admin/februari/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
