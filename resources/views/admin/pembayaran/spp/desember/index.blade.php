...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-desember</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP desember Index</button>
	<a href="{{url('admin/desember/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/desember/kwitansiThermo', $desember->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/desember/kwitansiTinta', $desember->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/desember', $desember->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/desember', $desember->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/desember', $desember->id)])
			</div>
			</td>
			<td>{{$desember->id_santri}}</td>
			<td>{{$desember->nama_santri}}</td>
			<td>{{$desember->sts_ads_spp}}</td>
			<td>Rp.{{number_format($desember->nominal)}}</td>
			<td>{{$desember->waktu}}</td>
			<td>@if($desember->status == 'lunas')
			<button type="button" class="btn btn-success">{{$desember->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$desember->username}}</td>
			<td>{{$desember->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP desember Tunggakan</button>
	<a href="{{url('admin/desember/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
