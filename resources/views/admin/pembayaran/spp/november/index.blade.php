...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-november</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP november Index</button>
	<a href="{{url('admin/november/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/november/kwitansiThermo', $november->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/november/kwitansiTinta', $november->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/november', $november->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/november', $november->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/november', $november->id)])
			</div>
			</td>
			<td>{{$november->id_santri}}</td>
			<td>{{$november->nama_santri}}</td>
			<td>{{$november->sts_ads_spp}}</td>
			<td>Rp.{{number_format($november->nominal)}}</td>
			<td>{{$november->waktu}}</td>
			<td>@if($november->status == 'lunas')
			<button type="button" class="btn btn-success">{{$november->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$november->username}}</td>
			<td>{{$november->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP november Tunggakan</button>
	<a href="{{url('admin/november/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
