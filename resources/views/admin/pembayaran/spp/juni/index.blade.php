...
@section('content')
<div class="container">
<p class="h1" align="center">SPP-juni</p>
<div class="row">
...
<div class="tab-content" id="nav-tabContent">
	...
	<hr>
	<button type="button" class="btn btn-success">Data SPP juni Index</button>
	<a href="{{url('admin/juni/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
			...
			<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
			  <div class="btn-group" role="group">
			    ...
			    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
			      <a class="dropdown-item" href="{{url('admin/juni/kwitansiThermo', $juni->id)}}">Print Thermo (58mm)</a>
			      <a class="dropdown-item" href="#" >Print Thermo (80mm)</a>
			      <a class="dropdown-item" href="{{url('admin/juni/kwitansiTinta', $juni->id)}}" >Print Tinta</a>
			    </div>
			  </div>
			</div>

			<div class="btn-group">
			<a href="{{url('admin/juni', $juni->id)}}" class="btn btn-dark float-right"><i class="fa fa-info"></i></a>
			<a href="{{url('admin/juni', $juni->id)}}/edit" class="btn btn-warning float-right"><i class="fa fa-edit"></i></a>
			@include('admin.template.utils.delete', ['url' => url('admin/juni', $juni->id)])
			</div>
			</td>
			<td>{{$juni->id_santri}}</td>
			<td>{{$juni->nama_santri}}</td>
			<td>{{$juni->sts_ads_spp}}</td>
			<td>Rp.{{number_format($juni->nominal)}}</td>
			<td>{{$juni->waktu}}</td>
			<td>@if($juni->status == 'lunas')
			<button type="button" class="btn btn-success">{{$juni->status}}</button>
			@else
			<button type="button" class="btn btn-danger">belum lunas</button>
			@endif
			</td>
			<td>{{$juni->username}}</td>
			<td>{{$juni->keterangan}}</td>
		 </tr>
		@endforeach
		</tbody>
	</table>
...
<div class="tab-pane fade" id="nav-tunggakan" role="tabpanel" aria-labelledby="nav-tunggakan-tab">
	...
	<br><hr>
	 <button type="button" class="btn btn-danger">Data SPP juni Tunggakan</button>
	<a href="{{url('admin/juni/create')}}" class="btn btn-dark float-right"><i class="fa fa-plus"></i> Tambah</a>
	<div class="card-body">
		<table class="table table-datatable">
			<thead>
